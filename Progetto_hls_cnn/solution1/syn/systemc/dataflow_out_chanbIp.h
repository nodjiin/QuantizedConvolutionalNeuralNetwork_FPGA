...
#ifndef _dataflow_out_chanbIp_H_
#define _dataflow_out_chanbIp_H_

#include <systemc>
#include <iostream>
#include <fstream>

#include "dataflow_out_chanbIp_memcore.h"

SC_MODULE(dataflow_out_chanbIp) {
...
protected:
    ...
    dataflow_out_chanbIp_memcore* dataflow_out_chanbIp_memcore_U;
...
public:
    ...
    SC_CTOR(dataflow_out_chanbIp) : m_trace_file(0) {
        SC_METHOD(proc_memcore_addr);
        sensitive << i_address0 << t_address0 << iptr << tptr;

        dataflow_out_chanbIp_memcore_U = new dataflow_out_chanbIp_memcore("dataflow_out_chanbIp_memcore_U");
        dataflow_out_chanbIp_memcore_U->clk      ( clk );
        dataflow_out_chanbIp_memcore_U->reset    ( reset );
        dataflow_out_chanbIp_memcore_U->ce0      ( i_ce0 );
        dataflow_out_chanbIp_memcore_U->we0      ( i_we0 );
        dataflow_out_chanbIp_memcore_U->address0 ( memcore_iaddr );
        dataflow_out_chanbIp_memcore_U->d0       ( i_d0 );
        dataflow_out_chanbIp_memcore_U->q0       ( i_q0 );

        dataflow_out_chanbIp_memcore_U->ce1      ( t_ce0 );
        dataflow_out_chanbIp_memcore_U->we1      ( t_we0 );
        dataflow_out_chanbIp_memcore_U->address1 ( memcore_taddr );
        dataflow_out_chanbIp_memcore_U->d1       ( t_d0 );
        dataflow_out_chanbIp_memcore_U->q1       ( t_q0 );
        ...
        if (dump_vcd) {
            m_trace_file = sc_create_vcd_trace_file( "dataflow_out_chanbIp.trace");
            // ports
