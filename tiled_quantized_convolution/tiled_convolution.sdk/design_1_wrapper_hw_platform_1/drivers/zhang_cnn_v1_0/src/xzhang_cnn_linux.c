...
typedef struct {
    ...
    XZhang_cnn_uio_map maps[ MAX_UIO_MAPS ];
} XZhang_cnn_uio_info;

/***************** Variable Definitions **************************************/
static XZhang_cnn_uio_info uio_info;
...
static int uio_info_read_name(XZhang_cnn_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XZhang_cnn_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XZhang_cnn_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XZhang_cnn_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XZhang_cnn_Initialize(XZhang_cnn *InstancePtr, const char* InstanceName) {
	XZhang_cnn_uio_info *InfoPtr = &uio_info;
